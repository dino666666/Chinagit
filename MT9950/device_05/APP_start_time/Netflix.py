...
def Netflix_run(count):
    ...
    csv_w = csv_write(path=picturepath + 'Netflix.csv')
    #脚本调试信息
    ...
    while True:
        ...
        while True:
            ...
            if APP_start >= 20 and float(score)>= 0.98:
                logO.info(score)
                APP_start += 1
            ...
            if APP_start == 10:
                logO.info(score)
            if APP_wait == 1 and float(score) <= 0.8:
                frame_num += 1

            if frame_num >= 2 and float(score) <= 0.4 and float(brightness(frame, type=1)) <= 20:
            # if frame_num >= 2 and float(score)  <= 0.4:
                print(brightness(frame,type=1))
                logO.info(score)
                T1 = time.time()
                ...
                break
            if APP_start == 10 and float(score)>= 0.98:
                if time.time() - T0 >= 60:
                    logO.info('APP start suceess')
                    cv2.destroyAllWindows()
                    print('cv2.destroyAllWindows()')
        ...
        li = []
        T = (T1 - T0)*1000
        logO.info('Netflix Start time:' + str(T) + 'ms')
        li.append(str(num))
        li.append(str(T))
        print(li)
        logO.info(li)
