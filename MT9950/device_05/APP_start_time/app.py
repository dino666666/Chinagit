...
for settings_num in range(40):
    ...
    settings = adb.order(' am start -W com.android.tv.settings/com.android.tv.settings.MainSettings')

    time_read = settings.readline()
    while time_read:
        logO.info(time_read)
        time_read = settings.readline()
    time.sleep(3)
...
num = 0
for filebrowser_num in range(40):
    flag = 0
    # 判断是否需要跳出循环，结束运行中的脚本
    if not q.empty():
        if q.get() == 'q':
    ...
    settings = adb.order(' am start -W com.oneplus.tv.filebrowser/.ui.activity.MainActivity')
